...
import { deleteFile, decode, addCompare } from '@/config'
...
export default {
  ...
  methods: {
    ...
    //       this.uploadbuyerDocs[i].uploader = this.admin.fname + ' ' + this.admin.lname
    //     }
    ...
    //       this.uploadsellerDocs[i].uploader = this.admin.fname + ' ' + this.admin.lname
    //     }
    ...
    async getDeal () {
      ...
      if (response.data.status === 'success') {
        ...
        let [poster, buyer, seller] = await Promise.all([this.getName(this.deal.poster), this.getAddressName(this.deal.buyer), this.getAddressName(this.deal.seller), this.getDocs(), this.getPartners()])
