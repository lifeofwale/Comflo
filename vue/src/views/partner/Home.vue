                ...
                <tbody>
                    <tr v-for="(deal, index) in deals" :key="index">
                        <template>
                            ...
                            <td v-else-if="deal[9] == '1'" class="text-info">Active</td>
                            <td v-else-if="deal[9] == '2'" class="text-primary">Paid</td>
                            <td v-else-if="deal[9] == '3'" class="text-success">Completed</td>
                            <td v-else class="text-danger">Terminated</td>
                            <td class="">{{deal[10]}}</td>
                            <td>
                                <a v-link="'/partner/deals/'+deal[0]+'/edit'">
                                    Edit
                                </a>
                            </td>
                            <td>
                                <a v-link="'/partner/deals/'+deal[0]">
                                    View
                                </a>
                            </td>
                        </template>
...
import uaerApi from '@/api/user'
import Header from '@/components/Header'
import { addCompare } from '@/config'
...
export default {
  ...
  methods: {
    async getDeals () {
      ...
      let response = await uaerApi.getDeals()
